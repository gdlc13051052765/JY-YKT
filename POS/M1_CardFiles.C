...
uchar	ReadCardCommonDatas_M1 (void)
{
	...
	if (status)
		return	CARD_NOCARD;
	if (BytesComp(RdCardDatasBuffer,MatchCode,4))
...
//	status=memcmpself(RdCardDatasBuffer+9,SysTimeDatas.TimeString,3);
//	if (status==0xff)
...
extern u8 PurseSectorDatas[16];
extern u8 Flag_Overdata;
extern uchar	PurseBTNumNew[2];//新的补贴序列号
extern uchar	OldCardBufferBak[16];//原卡数据备份
extern uchar btdata;
uchar	ReadCardBalanceDatas_M1( uchar RdSector)//读出卡金额及余额比较
{
	...
	{//子钱包的有效期
		...
		{
			status=memcmpself(RdCardDatasBuffer+12,SysTimeDatas.TimeString,3);
			if (status==0xff)
			{
				Flag_Overdata = 1;
				if(btdata == 0)
	...
	memcpy(PurseSectorDatas,RdCardDatasBuffer,16);
	for (i=0;i<5;i++)
	{
	...
	else
	{
		...
		Date2[2]=i;
		status=memcmpself(Date1,Date2,4);//日期比较
		if (status==1)
			bitUseBlock1=1;	
		else
			bitUseBlock1=0;						
	...
	memcpy(dispBalance,OldBalance,4);
	...
	memcpy(PurseConsumCount,RdCardDatasBuffer+status+11,2);//钱包流水号
	memcpy(PurseBT_Num,RdCardDatasBuffer+status+13,2);//控制序列号
	memcpy(PurseContrlNum,RdCardDatasBuffer+status+13,2);//控制序列号
	memcpy(OldCardBufferBak,RdCardDatasBuffer+status,16);
...
{//bit == 0 消费状态;bit == 1取消消费回写原来金额
	...
	if (!bbit)//消费写入
	{
	  //钱包余额数据
		...
		{
			if (memcmpself(PurseConsumDateTime,SysTimeDatas.TimeString+1,1))//不是同一月
		...
		{
			if (memcmpself(PurseConsumDateTime,SysTimeDatas.TimeString+1,2))//不是同一天
				ChgUlongToBCDString(CurrentConsumMoney,Buffer+7,4);//钱包的日累计消费额
			else
			{
				iii=PurseSumConsumMoney+CurrentConsumMoney;
		...
		{
			memcpy(Buffer+11,PurseConsumCount,2);//卡流水号		
		}
	...
	if (!status)
	{
		for (i=0;i<5;i++)
		{
			...
			if (!status)
			{
				...
				if (!status)
				{
					status=memcmpself(PurseWrBufferBak+1,BufferBak,16);
					if (!status)
						break;
				}
			}
	...
	if (!status)
	{
		for (i=0;i<5;i++)
		{
			...
			if (!status)
			{
				...
				if (!status)
				{
					status=memcmpself(MainWrBufferBak+1,BufferBak,16);
					if (!status)
							break;
				}
			}
	...
	if (!status)
	{
		...
		if(!bbit)
		{
			...
			{	
				...
				if(!status)
				{
					status=memcmpself(Buffer,PurseDatas_Info,16);
					if(status)
					{
						status=memcmpself(Buffer,PurseWrBufferBak+1,16);
						if(status)//数据已经发生变化
						...
						else
						{
							...
							if(!status)
							{
								status=memcmpself(Buffer,DaySumMoneyDatasBak,16);
								if(status)
									return CARD_SAME_ERROR;
								status=CardAuthKeySub(1,PurseWrBufferBak[0]);
							}
		...
		{//只写累计时验证数据
			...
			{		
				...
				{	
					...
					if(!status)
					{
//						status=memcmpself(Buffer,Buffer1,4);
//						if(status==1)
//							return CARD_SAME_ERROR;
//						else
//							status=0;
