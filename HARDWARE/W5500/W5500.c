...
#include "W5500.h"
extern unsigned char ResetEnc28j60Num;
...
* 说明    : 无
*******************************************************************************/
extern unsigned char S1_DIPR[4];
extern unsigned char S1_PORT[2];
void socket2senddata(unsigned char *dat_ptr, unsigned short size)
{
	...
	if((Read_W5500_SOCK_1Byte(s,Sn_MR)&0x0f) != SOCK_UDP)//如果Socket打开失败
	{		
		Write_W5500_SOCK_4Byte(s, Sn_DIPR, S1_DIPR);//设置目的主机IP  		
		Write_W5500_SOCK_2Byte(s, Sn_DPORTR, S1_PORT[0]*256+S1_PORT[1]);//设置目的主机端口号				
	}
...
void Socket_Init(SOCKET s)
{
	...
	switch(s)
	{
		...
		case 1:
			//设置端口0的端口号
			Write_W5500_SOCK_2Byte(1, Sn_PORT, S0_Port[0]*256+S0_Port[1]+5);	
			Socket_UDP(1);
			break;
...
void Read_W5500_nByte(unsigned short reg, unsigned char *dat_ptr, unsigned short size)
{
	...
	for(i=0;i<size;i++)
	{
		dat_ptr[i] = Read_W5500_1Byte(reg+i);
	}
}
...
void chekc_w5500()
{
	...
	Read_W5500_nByte(GAR, tmpdata, 4);
	status += memcmp((const void *)tmpdata,(const void *)Gateway_IP,4);
	Read_W5500_nByte(SUBR, tmpdata, 4);
	status += memcmp((const void *)tmpdata,(const void *)Sub_Mask,4);
	Read_W5500_nByte(SHAR, tmpdata, 6);
	status += memcmp((const void *)tmpdata,(const void *)Phy_Addr,6);
	Read_W5500_nByte(SIPR, tmpdata, 4);
	status += memcmp((const void *)tmpdata,(const void *)IP_Addr,4);
